        ...
        if not self.peer_exists(name):
            if self.bigip.device.get_lock():
                ...
                root_mgmt_dict = {'root_device_name': local_device,
                                  ...
                                  local_mgmt_address}
                local_md = self.bigip.device.get_metadata(local_device)
                if local_md is not None and 'root_device_name' in local_md:
                    ...
                    if md_device_name:
                        if not md_device_name == local_device:
                            raise exceptions.BigIPClusterPeerAddFailure(
                                ...
                                + ' device: %s'
                                % local_md['root_device_name'])
                self.bigip.device.update_metadata(local_device, root_mgmt_dict)
                ...
                raise exceptions.BigIPDeviceLockAcquireFailed(
                    ...
                    % os.path.basename(self.get_local_device_name())
                    )

...
class BigipVE(object):
    ...
    def __init__(self, creds, args):
        ...
        image = self.nova_tenant.get_image(name=args.bigip_image)
        if not image:
        ...
        if 'nova_flavor' in image.metadata:
            ...
            flavor = self.nova_tenant.get_flavor(flavor_name)
            if flavor:
        ...
        if not flavor_confirmed:
            ...
            if args.bigip_flavor:
                flavor = self.nova_tenant.get_flavor(args.bigip_flavor)
                if flavor:
                    flavor_confirmed = True
                    BIG_IP_FLAVOR['name'] = flavor.name
                    BIG_IP_FLAVOR['ram'] = flavor.ram
                ...
                flavor = self.nova_tenant.get_flavor(BIG_IP_FLAVOR['name'])
                if not flavor:
                    flavor = self.nova_tenant.create_flavor(
                        BIG_IP_FLAVOR['name'],
                        BIG_IP_FLAVOR['ram'],
                        BIG_IP_FLAVOR['vcpus'],
                        BIG_IP_FLAVOR['disk']
        ...
        if not self.args.no_floating_ips:
            ...
            self.ext_net = self.neutron_network_tenant.get_network(
                ext_network_def['network_name'])
            if not self.ext_net:
                exnets = self.neutron_network_tenant.get_external_networks()
                if len(exnets) < 1:
        ...
        if args.bigip_mgmt_network or args.bigip_networks:
            nc = self.neutron_network_tenant
    ...
    def build_instance_nic_list(self):
        ...
        for logical_net_name in self.logical_net_names:
            ...
            net = self.neutron_network_tenant.get_network(net_name)
            if not net:
                raise Exception(
                    "Cannot get network: network_name: %s" % net_name)
    ...
    def set_up(self, floating_ip_retry=60):
        ...
        for bigip_index in range(int(self.args.bigip_index),
            ...
            server = self.nova_tenant.get_instance(instance_name)
            if not server:
                userdata = None
                if bigip_index in userdatas:
                    userdata = userdatas[bigip_index]
                    print "using userdata: %s" % userdata
                else:
                ...
                if self.args.meta:
                    meta = self.args.meta
                instances.append(self.nova_tenant.create_instance(
            ...
            if not self.args.no_floating_ips:
                ...
                for instance in instances:
                    ...
                    # as of 4.5 will also discover the bigip via its external
                    # network address for the bigiq openstack connector
                    if 'external' in self.bigip_net_def:
                        port = self.neutron_network_tenant.wait_instance_port(
                            instance,
                            self.bigip_net_def['external']['network_name']
                        )
                        floating_ip_addr = self.neutron_floating_ip_tenant.\
                            associate_port_floating_ip(
                                self.ext_net, port['id']
                            )
                        if not test_connectivity(floating_ip_addr, PORT_SSH,
                                                 floating_ip_retry):
                            raise Exception(
                                'Instance connectivity test failed.')
                    # Using the mgmt network, the preferred default behavior,
                    # is used as well.
                    port = self.neutron_network_tenant.wait_instance_port(
                        instance,
    ...
    def tear_down(self):
        ...
        for bigip_index in range(
                int(self.args.bigip_index),
                int(self.args.bigip_index)+int(self.args.num_bigips)):
            instance_name = 'bigip%d' % bigip_index
            instance = self.nova_tenant.get_instance(instance_name)
            if instance:
                # see comments in set_up() regarding use of floating ips on
                # on both mgmt and external networks
                port = self.neutron_network_tenant.get_instance_port(
                    instance, self.bigip_net_def['mgmt']['network_name'])
                if port:
                    floating_ip_addr = self.neutron_floating_ip_tenant.\
                        get_port_floating_ip_addr(port['id'])
                    if floating_ip_addr:
                        self.neutron_floating_ip_tenant.delete_floating_ip(
                            floating_ip_addr)

                port = self.neutron_network_tenant.get_instance_port(
                    instance, self.bigip_net_def['external']['network_name'])
                if port:
                    floating_ip_addr = self.neutron_floating_ip_tenant.\
                        get_port_floating_ip_addr(port['id'])
                    if floating_ip_addr:
                ...
                self.nova_tenant.delete_instance(instance)
